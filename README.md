...
- все 13 039 description ID предметов и блоков из живого реестра сервера, включая
  400 ключей, пропущенных самими модами;
...
Аудит подтверждает покрытие 18 316 из 18 316 языковых ключей, 13 039 из 13 039
description ID живого реестра и отсутствие непроверенных английских заглушек.
Неизменяемые названия модов, имена собственные, команды, идентификаторы и единицы
измерения учитываются как проверенные литералы. Подробные числа записываются в
`translation/mod_inventory.tsv`.
...
На сервере доступна команда `/ai <вопрос>`. Помощник получает релевантные выдержки из
квестов, KubeJS-рецептов, модлиста и материалов перевода, а также координаты, измерение,
здоровье и предмет в руке вызвавшего игрока. Запрос выполняется через локальный мост на
`127.0.0.1`, поэтому сетевой поток Minecraft не ждёт ответа модели.

Для включения выполните `./serverctl ai-configure`: команда без эха запросит API-ключ,
сохранит его в `.server.env` с правами `600` и перезапустит мост. При необходимости
ограничьте доступ через `AI_ALLOWED_PLAYERS`, затем один раз перезапустите Minecraft.
Файл `.server.env` не попадает в Git.

Помощник не имеет shell-доступа и не может передать в консоль произвольную строку. Он
может предложить только команды из жёсткого белого списка. Любая команда показывается
игроку и исполняется лишь в течение 60 секунд после `/ai confirm`; `/ai cancel` отменяет
предложение. Состояние моста проверяется командой `./serverctl ai-status`.
