...
Для включения скопируйте `.server.env.example` в `.server.env`, укажите
`OPENAI_API_KEY`, ограничьте доступ через `AI_ALLOWED_PLAYERS` и перезапустите сервер.
Файл `.server.env` не попадает в Git; рекомендуется установить ему права `600`.
